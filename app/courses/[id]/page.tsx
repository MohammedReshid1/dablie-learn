...
import { getCategoryColorClasses, getCategoryHighlightColor } from "@/lib/category-colors"
...
export default function CourseDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Get category colors based on the assumed slug
  const colors = getCategoryColorClasses(course.categorySlug)
  const categoryHighlightBg = getCategoryHighlightColor(course.categorySlug)
            ...
            <Tabs defaultValue="overview" className="w-full">
              <TabsList className="grid w-full md:w-auto grid-cols-4 md:inline-flex md:h-10 items-center justify-center rounded-md bg-muted p-1 text-muted-foreground">
                <TabsTrigger
                  value="overview"
                  className={cn(
                    "inline-flex items-center justify-center whitespace-nowrap rounded-sm px-3 py-1.5 text-sm font-medium ring-offset-background transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:text-primary-foreground data-[state=active]:shadow-sm",
                    `data-[state=active]:${categoryHighlightBg}`
                  )}
                >
                  Overview
                </TabsTrigger>
